import Worker from "./artist.js?worker";

let artist: Worker | null = null;
let lastCustomCode = "";
let seeds: Array<Number> | null = null;
let index = [0, 0];
let ongoing = 0;
let artistTimeout: number | null = null;
let renderCache = {};
let schemaName: string | null = null;
let customChangeTimeout: number | null = null;
let editor = null;
...
function selectSchema(name) {
  ...
  if (schemaName) {
    let e = document.getElementById(
      schemaName == CUSTOM ? "customthumb" : `thumb-${schemaName}`
    );
...
function updateCustom() {
  ...
  let code =
    customCode.replace("°", " * Math.PI * 2").slice(0, -suffix.length) +
    "} draw";
...
function customChanged() {
  ...
  customChangeTimeout = window.setTimeout(() => {
    customChangeTimeout = null;
    rerender();
  }, 1000);
}
...
function resetArtist() {
  ...
  artist = new Worker(Worker, { type: "module" });
